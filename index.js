...
const sendAccounts = (currentUserName, docs, token) => {
  ...
  const formattedDocs = docs.map((doc) => {
    ...
    if (
      avatar &&
      !fs.existsSync(path.join(__dirname, "uploads/avatars", avatar))
    ) {
      avatar = null;
...
app.get("/avatar/:id", (req, res) => {
  ...
  const file = fs.createReadStream(
    path.join(__dirname, "uploads/avatars", id),
    (err) => console.log(err)
  );
  file.pipe(res);
});
...
app.get("/preview/:id", (req, res) => {
  const { id } = req.params;
  const file = fs.createReadStream(
    path.join(__dirname, "uploads/previews", id),
    (err) => console.log(err)
  );
  file.pipe(res);
});
...
app.post(
  ...
  (req, res) => {
    ...
    if (req.files.video[0].mimetype !== "video/mp4") {
      ...
      res
        .status(406)
        .send(`${req.files.video[0].mimetype} mimetype is not supported`);
      if (req.files.preview) {
        deleteUploadedFile(req.files.preview[0].filename, "uploads/previews");
      }
      return;
    ...
    UserModel.findOne({ name: accountInfo.name })
      .then((accountDoc) => {
        const { title, description } = req.body;
        const videoDoc = VideoModel({
          title,
          description: description ? description : null,
    ...
    .then((docs) => {
      if (token) {
        const handledDocs = docs.map((doc) => {
          ...
          if (
            preview &&
            !fs.existsSync(path.join(__dirname, "uploads/previews", preview))
          ) {
            doc.preview = null;
            preview = null;
            doc.save();
          ...
          let liked = false,
            disliked = false;
          const votedByUser = votedBy.find(
            (vote) => vote.voter === accountInfo.name
          );
          if (votedByUser) {
      ...
      res.send([
        ...docs
          .map((doc) => {
            ...
            if (
              !fs.existsSync(path.join(__dirname, "uploads/videos", source))
            ) {
              VideoModel.findByIdAndDelete(id);
              return;
            }
...
app.post("/vote-video/:videoId", async (req, res) => {
  ...
  const voteDoc = videoDoc.votedBy.find(
    (doc) => doc.voter === accountInfo.name
  );
  if (voteDoc) {
    const index = videoDoc.votedBy.findIndex(
      (doc) => doc.voter === accountInfo.name
    );
    if (likePressed) {
      if (disliked) {
