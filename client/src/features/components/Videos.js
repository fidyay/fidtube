...
import NoVideos from "./NoVideos.js";
...
const Videos = ({ searchInfo, author, self, accountPage }) => {
  ...
  if (accountPage) {
    videos = allVideos.filter(
      (video) =>
        video.author === author &&
        (video.title.includes(searchInfo) ||
          video.description.includes(searchInfo))
    );
  } else {
    videos = allVideos.filter(
      (video) =>
        video.title.includes(searchInfo) ||
        video.description.includes(searchInfo) ||
        video.author.includes(searchInfo)
    );
  }
  ...
  let margins =
    windowWidth > 640
      ? (windowWidth - minScreenSize * numberOfVideos) / (numberOfVideos + 1)
      : 0;
